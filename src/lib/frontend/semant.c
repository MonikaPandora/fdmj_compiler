...
static S_table METHOD_INHERIT_MAP;
...
inline static void inherit_offset(E_enventry coff, E_enventry poff){
  ...
  _for_in_table(iter, poff->u.off.local_offtbl){
    // fprintf(stderr, "cur:%p, prev:%p\n", iter->key, iter->prevtop);
    // fprintf(stderr, "%s %ld\n", S_name((S_symbol)iter->key), (__int64_t)iter->value);
    // fflush(stderr);
...
inline static void inherit_mtds(FILE* out, E_enventry child, E_enventry parent){
  // check and inherit methods from parent class
  // shallow copy
  S_table child_mtbl = child->u.cls.mtbl;
  S_table parent_mtbl = parent->u.cls.mtbl;
  ...
  _for_in_table(iter, parent_mtbl){
    ...
    if(men){
      ...
      // build a map from child method to parent method, in order to get right location of child method
      S_enter(METHOD_INHERIT_MAP, S_link(S_Symbol(child->u.cls.cd->id), S_Symbol(pa_md->id)), 
                                  S_link(S_Symbol(parent->u.cls.cd->id), S_Symbol(pa_md->id)));
    }
    INFO("[%s] Inherited.\n", pa_md->id);
  }
}
  ...
  switch(exp->kind){
    ...
    case A_newObjExp: {
      ...
      *has_location = 0;
      ...
      return Expty(
        Tr_NewObjExp(S_Symbol(exp->u.v), off_en, ce, METHOD_INHERIT_MAP),
...
static T_funcDeclList transA_Classes(FILE* out, A_classDeclList cdl){
  ...
  _for_in_table(iter, CENV){
    ...
    _for_in_table(mtds, cls->u.cls.mtbl){
      ...
      if(S_look(METHOD_INHERIT_MAP, S_link(cur_cls, cur_mtd)))continue;
