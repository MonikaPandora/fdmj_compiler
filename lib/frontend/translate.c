...
Tr_exp Tr_NewObjExp(S_symbol cls_name, E_enventry offset_entry, E_enventry cls){
  ...
  _for_in_table(iter, cls->u.cls.mtbl){
    ...
    T_exp offset = T_IntConst((int)(unsigned long long)S_look(offset_entry->u.off.local_offtbl, S_Symbol(me->u.meth.md->id)));
    T_exp addr = offset->u.CONST.i ? T_Binop(T_plus, T_Temp(t), offset) : T_Temp(t);
    seqBufPush(Tr_Nx(
      T_Move(
        T_Mem(addr, T_int),
        T_Name(Temp_namedlabel(S_name(S_link(cls_name, S_Symbol(me->u.meth.md->id)))))
      )
    ));
  }
