...
E_enventry E_OffsetEntry(unsigned long long local_size, S_table offset_tbl){
  E_enventry entry = checked_malloc(sizeof(*entry));
  entry->kind = E_offsetEntry;
  entry->u.off.local_size = local_size;
  entry->u.off.local_offtbl = offset_tbl;
  return entry;
}

// E_record E_varRecord(bool need_init, float num, unsigned offset){
//   E_record r = checked_malloc(sizeof(*r));
//   r->kind = E_classVar;
//   r->need_init = need_init;

// }

// E_record E_mtdRecord(bool need_init, string v, unsigned offset){
