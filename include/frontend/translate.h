...
Tr_exp Tr_NewObjExp(S_symbol cls_name, E_enventry offset_entry, E_enventry cls);
...
typedef struct seqBuf_* seqBuf;

static seqBuf seqbuf = NULL;
...
#define Sequence(...) SequenceImpl(__VA_ARGS__, NULL)

Tr_exp SequenceImpl(T_stm first, ...);

#define SequenceFromBuffer() SequenceFromBufferImpl(0)
...
// Tr_exp Tr_cls_intInit(unsigned off, int val);

// Tr_exp Tr_cls_floatInit(unsigned off, float val);
